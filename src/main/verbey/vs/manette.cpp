...
// libere espace utilis� pour infos manette
void ma_detruireInput(Input *input)
{
	if (input->joystick != NULL)
	{
		SDL_JoystickEventState(SDL_DISABLE); // desactive les �v�nts du joystick

		for (int i = 0; i<input->nombreJoysticks; i++) // tant qu'un joystick est connect�
		{
...
void ma_updateEvent(Input *input)
{
	...
	static SDL_Event evenements; // en static car appelle plusieurs fois par seconde
								 ...
								 // actions
	if (input->nombreJoysticks != 0) // on ne v�rifie qu'une seule fois qu'un joystick existe
	{
		while (SDL_PollEvent(&evenements))
		{
			// on v�rifie qu'un des joysticks est concern�
